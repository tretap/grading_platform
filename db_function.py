...
def create_classroom(name,discription):
	Session = sessionmaker(bind=engine)
	session = Session()

	class_ = Classboom(name, discription)
	session.add(class_)
...
def create_assigment(id_class, name, description, scoring_type, assignment_score, open_time, close_time):
	...
	assignment_info = Classboard_db_assignment(id_class,(inspect(_assignment).identity)[0])
